...
namespace WeatherApp.Application.UseCases
{
    public class FetchWeatherDataUseCase
    {
        ...
        public async Task<WeatherResponse> ExecuteAsync(string locationName)
        {
            try
            {
                // Use the injected WeatherDataService to fetch weather data
                WeatherResponse weatherData = await _weatherDataService.GetWeatherDataAsync(locationName);
